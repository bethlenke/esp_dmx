...
rdm_uid_t rdm_get_uid(dmx_port_t dmx_num)
{
  ...
  if (driver->rdm.uid == 0)
  {
    ...
    driver->rdm.uid |= (rdm_uid_t)RDM_ManUUID << 32;//RDM_DEFAULT_MAN_ID << 32;
  }
...
size_t rdm_send_set_param_response(dmx_port_t dmx_num, rdm_uid_t uid, uint8_t tn, rdm_pid_t pid, uint16_t sub_device, const void *pd, size_t pdl)
{
  RDM_CHECK(dmx_num < DMX_NUM_MAX, 0, "dmx_num error");
  RDM_CHECK(dmx_driver_is_installed(dmx_num), 0, "driver is not installed");
